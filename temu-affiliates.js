...
// Parse and convert PKR string (e.g. "Rs.2,230" or "Rs.40,865") accurately to USD
function calculatePrices(pkrString) {
    if (!pkrString) return { dealPrice: "$0.00", originalPrice: "$0.00", savePct: "40%" };
    
    // Remove commas and extract clean numeric float
    let cleanStr = pkrString.toString().replace(/,/g, '');
    let numericValue = parseFloat(cleanStr.replace(/[^0-9.]/g, ''));
    
    if (isNaN(numericValue) || numericValue <= 0) {
        return { dealPrice: "$9.99", originalPrice: "$19.99", savePct: "50%" };
    }

    // Convert PKR to USD
    let usdDeal = (numericValue / pkrToUsdRate);
    let usdOriginal = usdDeal * 1.85; // Calculate estimated original list price (~45% off)
    let savePercent = Math.round(((usdOriginal - usdDeal) / usdOriginal) * 100);

    return {
        dealPrice: `$${usdDeal.toFixed(2)}`,
        originalPrice: `$${usdOriginal.toFixed(2)}`,
        savePct: `SAVE ${savePercent}%`
    };
}

// Default Fallback Image
const fallbackSolarImg = "https://images.unsplash.com/photo-1509391365360-2e959784a276?w=500&auto=format&fit=crop&q=80";
...
// Function to render Temu Products Slider
function renderTemuSlider(categoryKey, containerId) {
    ...
    container.innerHTML = `
        <div class="temu-slider-card" style="background-color: #0f172a; border: 1px solid #f97316; border-radius: 12px; padding: 18px; text-align: center; position: relative; box-shadow: 0 4px 15px rgba(249, 115, 22, 0.15);">
            ...
            <img src="${item.image}" alt="${item.title}" onerror="this.onerror=null; this.src='${fallbackSolarImg}';" style="width: 100%; height: 160px; object-fit: cover; border-radius: 8px; margin-bottom: 12px; background-color: #1e293b;">
...
// Function to handle Next / Prev Slide Actions
