...
// Convert PKR price string (e.g. "Rs.8,437") to USD ($)
...
const temuData = {
    "solar-energy": [
        ...
        },
        {
            ...
            image: "https://images.unsplash.com/photo-1548611716-30018596f2a2?w=500&q=80"
        },
        {
...
function renderTemuSlider(categoryKey, containerId) {
    ...
    const usdPrice = formatPriceToUSD(item.rawPrice);

    container.innerHTML = `
        <div class="temu-slider-card" style="background-color: #0f172a; border: 1px solid #f97316; border-radius: 12px; padding: 18px; text-align: center; position: relative;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                <span style="background: #f97316; color: white; padding: 3px 8px; font-size: 11px; border-radius: 4px; font-weight: bold; text-transform: uppercase;">Temu Physical Deal</span>
                <span style="color: #94a3b8; font-size: 12px; font-weight: bold;">Deal ${index + 1} of ${products.length}</span>
            </div>
            
            <img src="${item.image}" alt="${item.title}" style="width: 100%; height: 160px; object-fit: cover; border-radius: 8px; margin-bottom: 12px;">
            
            <h4 style="color: #f8fafc; font-size: 14px; margin: 0 0 8px 0; line-height: 1.4; min-height: 38px;">${item.title}</h4>
            
            <div style="margin-bottom: 12px;">
                <span style="color: #10b981; font-size: 20px; font-weight: bold;">${usdPrice}</span>
            </div>
